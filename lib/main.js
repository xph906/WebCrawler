...
loggingDB.executeSQL(create_redirect_table,false);
...
PageCrawler.prototype = {
	...
	crawlHandler : function(tab){
		if(tab.original){
			console.log("REDIRECT FOUND: From "+tab.original+" to "+tab.url);
			var update = {};
			update['from_url'] = loggingDB.escapeString(tab.original.toLowerCase());
			update['to_url'] = loggingDB.escapeString(tab.url.toLowerCase());
			var statement = loggingDB.createInsert("redirects",update);
			loggingDB.executeSQL(statement,true);
		}
		tab.original = tab.url;
		//console.log(tab.url+" on ready");
	}	
};
...
function Test(){
	tabs.open({
		url: "http://garuda.cs.northwestern.edu/xpan/redirect/meta_redirect3.html",
...
function onReady(tab){
	if(tab.original){
		console.log("From "+tab.original+" to "+tab.url);
	}
	tab.original = tab.url;
	console.log(tab.url+" on ready");
}

function onOpenFun(tab) {
